...
const CONSTRUCTOR_SECRET = /* @__PURE__ */ Symbol("model-constructor-secret");
...
${associationOwner._getAssociationDebugList()}`);
  ...
  static async count(options) {
    ...
    if (options.group && options.countGroupedRows) {
      const query = (0, import_string.removeTrailingSemicolon)(this.queryGenerator.selectQuery(this.table, options));
      const queryCountAll = this.queryGenerator.generateCountAllQuery(query);
      const result2 = await this.sequelize.query(queryCountAll);
      const count = Number(result2[0][0].count || result2[0][0].COUNT);
      return count;
    }
    let col = options.col || "*";
    if (options.include) {
  ...
  static async bulkCreate(records, options = {}) {
    ...
    const recursiveBulkCreate = async (instances2, options2) => {
      ...
      if (options2.ignoreDuplicates && model.sequelize.dialect.supports.inserts.ignoreDuplicates === false) {
        throw new Error(`${dialect} does not support the ignoreDuplicates option.`);
      }
      if (options2.updateOnDuplicate && !model.sequelize.dialect.supports.inserts.updateOnDuplicate) {
  ...
  static async destroy(options) {
    ...
    if (modelDefinition.timestampAttributeNames.deletedAt && !options.force) {
      options.type = import_enums.QueryTypes.BULKUPDATE;
      const attrValueHash = {};
  ...
  static async restore(options) {
    ...
    (0, import_model_internals.setTransactionFromCls)(options, this.sequelize);
    options.type = import_enums.QueryTypes.RAW;
    options.model = this;
    (0, import_format.mapOptionFieldNames)(options, this);
    if (options.hooks) {
  ...
  static [/* @__PURE__ */ Symbol.for("nodejs.util.inspect.custom")]() {
    return this.name;
  }
