...
class AbstractQueryInterfaceTypeScript {
  ...
  async tableExists(tableName, options) {
    ...
    const out = await this.sequelize.query(sql, { ...options, type: import_enums.QueryTypes.SELECT });
    return out.length === 1;
  }
  /**
  ...
  async describeTable(tableName, options) {
    ...
    const queryOptions = {
      ...options,
      type: import_enums.QueryTypes.DESCRIBE
    };
    try {
  ...
  async truncate(tableName, options) {
    ...
    const queryOptions = { ...options, raw: true, type: import_enums.QueryTypes.RAW };
  ...
  async addConstraint(tableName, options) {
    ...
    await this.sequelize.queryRaw(sql, { ...options, raw: true, type: import_enums.QueryTypes.RAW });
  }
  ...
  async showConstraints(tableName, options) {
    ...
    const rawConstraints = await this.sequelize.queryRaw(sql, {
      ...options,
      raw: true,
      type: import_enums.QueryTypes.SHOWCONSTRAINTS
    });
  ...
  async bulkDelete(tableOrModel, options) {
    ...
    return this.sequelize.queryRaw(sql, {
      ...bulkDeleteOptions,
      raw: true,
      type: import_enums.QueryTypes.DELETE
    });
  }
}
