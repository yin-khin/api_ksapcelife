...
var import_enums = require("../enums.js");
...
class AbstractQueryGenerator extends import_query_generator_typescript.AbstractQueryGeneratorTypeScript {
  ...
  insertQuery(table, valueHash, modelAttributes, options) {
    ...
    let bind;
    let bindParam;
    let parameterStyle = options?.parameterStyle ?? import_enums.ParameterStyle.BIND;
    let query;
    let valueQuery = "";
    let emptyQuery = "";
    let outputFragment = "";
    ...
    if ((0, import_get.default)(this, ["sequelize", "options", "prependSearchPath"]) || options.searchPath) {
      parameterStyle = import_enums.ParameterStyle.REPLACEMENT;
    }
    if (this.dialect.supports.EXCEPTION && options.exception) {
      parameterStyle = import_enums.ParameterStyle.REPLACEMENT;
    }
    if (parameterStyle === import_enums.ParameterStyle.BIND) {
      bind = this.dialect.supports.returnIntoValues && options.bind ? options.bind : /* @__PURE__ */ Object.create(null);
      bindParam = (0, import_sql.createBindParamGenerator)(bind, this.dialect.name === "oracle");
    }
  ...
  updateQuery(tableName, attrValueHash, where, options, columnDefinitions) {
    ...
    if (parameterStyle === import_enums.ParameterStyle.BIND) {
      ...
      bindParam = (0, import_sql.createBindParamGenerator)(bind);
    }
  ...
  */
    ...
    if (subQuery) {
      ...
      if (options.groupedLimit) {
        ...
        const baseQuery = `SELECT * FROM (${this.selectQuery(
          ...
          model
        ).replace(/;$/, "")}) ${this.#internals.getAliasToken()} sub`;
  ...
  _getAliasForField(tableName, field, options) {
    ...
    const tableNameString = typeof tableName === "string" ? tableName : void 0;
    const tableVariants = [];
    if (tableNameString) {
    ...
    const fieldVariants = /* @__PURE__ */ new Set();
    if (typeof field === "string") {
  ...
  _getAssociationOrderInfo(order, model, options) {
    if (!Array.isArray(order) || order.length < 2 || !(0, import_model_utils.isModelStatic)(model)) {
      return null;
    }
    const isDirectional = typeof order.at(-1) === "string" && VALID_ORDER_OPTIONS.includes(order.at(-1).toUpperCase());
    ...
    for (const part of associationParts) {
      let association;
      const previousBelongsToMany = previousAssociation instanceof import_belongs_to_many.BelongsToManyAssociation ? previousAssociation : null;
      const matchesPreviousThroughModel = previousBelongsToMany && (part === previousBelongsToMany.throughModel || (0, import_isPlainObject.default)(part) && part.model === previousBelongsToMany.throughModel && (part.as == null || part.as === previousBelongsToMany.fromTargetToThrough.as));
      if (matchesPreviousThroughModel) {
        association = previousBelongsToMany.fromTargetToThrough;
      } else if (part instanceof import_base.Association) {
        association = part;
      } else if (typeof part === "string" && currentModel?.associations?.[part]) {
        association = currentModel.associations[part];
      } else if ((0, import_model_utils.isModelStatic)(part)) {
        association = currentModel.getAssociationWithModel(part);
      } else if ((0, import_isPlainObject.default)(part) && part.model && (0, import_model_utils.isModelStatic)(part.model)) {
    ...
    return {
      attribute,
      replaceCount: associationParts.length + 1,
      // Number of elements to replace in the order array
      associationAliases,
      include,
      externalPath: associationAliases.join("."),
      // e.g., 'user.tasks' for nested associations
      internalPath: associationAliases.join("->")
      // e.g., 'user->tasks' used for internal aliasing
    };
  }
  /**
  ...
  generateThroughJoin(include, includeAs, parentTableName, topLevelInfo, options) {
    const isRootParent = !include.parent.association && include.parent.model.name === topLevelInfo.options.model.name;
    ...
    if (topLevelInfo.subQuery && !include.subQuery && include.parent.subQuery && isMinified) {
      const aliasCandidates = /* @__PURE__ */ new Set();
    ...
    } else if (topLevelInfo.subQuery && !include.subQuery && include.parent.subQuery && !isRootParent) {
  ...
  _generateSubQueryFilter(include, includeAs, topLevelInfo) {
    ...
    topLevelInfo.options.where = (0, import_sequelize.and)(
      topLevelInfo.options.where,
      new import_literal.Literal(["EXISTS (", query.replace(/;$/, ""), ")"].join(" "))
    );
  }
  /*
    ...
    if (Array.isArray(options.order)) {
      for (const rawOrder of options.order) {
        ...
        for (let order of orderEntries) {
          ...
          if (subQuery && Array.isArray(order) && order[0] && !Array.isArray(order[0]) && !(order[0] instanceof import_base.Association) && !(0, import_model_utils.isModelStatic)(order[0]) && !(0, import_model_utils.isModelStatic)(order[0].model) && !(typeof order[0] === "string" && model && model.associations !== void 0 && model.associations[order[0]])) {
          ...
          if (subQuery && associationOrderInfo?.include?.subQuery) {
            ...
            const alias = this._getAliasForField(associationOrderInfo.internalPath, aliasField, options) || aliasField;
            const aliasLiteral = new import_literal.Literal(this.quoteIdentifier(alias));
