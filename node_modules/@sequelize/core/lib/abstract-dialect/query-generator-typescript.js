  ...
  quoteTable(param, options) {
    ...
    if (options?.indexHints) {
      for (const hint of options.indexHints) {
        if (import_enums.IndexHints[hint.type]) {
          sql += ` ${import_enums.IndexHints[hint.type]} INDEX (${hint.values.map((indexName) => this.quoteIdentifier(indexName)).join(",")})`;
        } else {
          throw new Error(
    ...
    if (options?.tableHints) {
      ...
      for (const hint of options.tableHints) {
        if (import_enums.TableHints[hint]) {
          hints.push(import_enums.TableHints[hint]);
        } else {
          throw new Error(
